...
export default class GameScene extends Phaser.Scene {
  constructor() {
    ...
    // Burrow locations with corresponding key on keyboard, x and y coords.
  ...
  // Import the visual assets we will use throughout the game.
  preload() {
    this.load.setBaseURL("static");
    this.load.setPath("images/");
    this.load.image("background", "background.png");
    this.load.spritesheet("mole", "mole_spritesheet.png", {
      frameWidth: 198,
      frameHeight: 250,
    });
  }
  ...
  update() {
    if (timeLeft <= 0) {
      ...
      this.scene.start('EndScene', { score: score });
    }
  ...
  displayPauseScreen() {
    gameState.pauseOverlay = this.add.rectangle(0, 0, 480, 560, 0xffffff);
    gameState.pauseOverlay.alpha = 0.75;
    gameState.pauseOverlay.setOrigin(0, 0);

    gameState.pauseText = this.add.text(225, 280, "PAUSED").setColor("#000000");
